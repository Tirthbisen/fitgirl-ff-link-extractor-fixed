...
class FitgirlExtractorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("FitGirl FF Link Extractor - modified by TIRth ")
    ...
    def setup_ui(self):
        ...
        ttk.Label(
            input_frame, text="FitGirl Game URL:", font=("Arial", 10, "bold")
        ).grid(row=0, column=0, sticky="w", pady=5)

        self.url_var = tk.StringVar()
        self.url_var.set("https://fitgirl-repacks.site/grand-theft-auto-v/")
        self.url_entry = ttk.Entry(
            input_frame, textvariable=self.url_var, font=("Arial", 10)
        )
        self.url_entry.grid(row=1, column=0, columnspan=2, sticky="ew", pady=5)

        self.fetch_btn = ttk.Button(
            input_frame, text="1. Fetch Links", command=self.start_fetch_thread
        )
        ...
        checklist_container = ttk.LabelFrame(
            self.root, text="Found Parts (Uncheck unwanted)", padding="5 5 5 5"
        )
        ...
        self.scrollbar_list = ttk.Scrollbar(
            checklist_container, orient="vertical", command=self.canvas.yview
        )
        ...
        ttk.Button(controls_frame, text="Select All", command=self.select_all).pack(
            side="left", padx=2
        )
        ttk.Button(controls_frame, text="Deselect All", command=self.deselect_all).pack(
            side="left", padx=2
        )

        self.extract_btn = ttk.Button(
            controls_frame,
            text="2. Extract Selected",
        ...
        self.browser_combo = ttk.Combobox(
            controls_frame, textvariable=self.browser_var, state="readonly", width=18
        )
        self.browser_combo["values"] = ("Google Chrome",)
        ...
        output_frame = ttk.LabelFrame(
            self.root, text="Extracted Direct Links", padding="5 5 5 5"
        )
        ...
        self.text_area = tk.Text(
            output_frame, wrap="none", font=("Consolas", 9), height=10
        )
        self.text_area.grid(row=0, column=0, sticky="nsew")

        scrollbar_y = ttk.Scrollbar(
            output_frame, orient="vertical", command=self.text_area.yview
        )
        scrollbar_y.grid(row=0, column=1, sticky="ns")
        self.text_area.configure(yscrollcommand=scrollbar_y.set)

        scrollbar_x = ttk.Scrollbar(
            output_frame, orient="horizontal", command=self.text_area.xview
        )
        ...
        self.copy_btn = ttk.Button(
            btn_frame, text="📋 Copy All Links", command=self.copy_to_clipboard
        )
        self.copy_btn.pack(side="right")

        self.clear_btn = ttk.Button(
            btn_frame, text="Clear Output", command=self.clear_output
        )
    ...
    def _on_mousewheel(self, event):
        try:
            ...
            if widget == self.canvas or (
                widget and str(widget).startswith(str(self.scrollable_frame))
            ):
    ...
    def copy_to_clipboard(self):
        if links := self.text_area.get(1.0, tk.END).strip():
    ...
    def run_fetch(self, url):
        try:
            ...
            self.root.after(
                0,
                self.update_ui,
                "Network Error: Cannot reach FitGirl. Is your ISP blocking it? Try a VPN/Custom DNS.",
            )
    ...
    def populate_checkboxes(self, links):
        ...
        self.status_var.set(
            f"Found {len(links)} parts. Select what you need and click Extract."
        )
        self.fetch_btn.config(state="normal")
        self.extract_btn.config(state="normal")

    # --- Step 2: Extraction ---
    def get_browser_path(self, selected_browser="Google Chrome"):
        paths_to_check = [
    ...
    def start_extraction_thread(self):
        ...
        thread = threading.Thread(
            target=self.run_extraction, args=(selected_links,), daemon=True
        )
        thread.start()
    ...
    def run_extraction(self, links):
        ...
        self.root.after(
            0,
            self.update_ui,
            f"Initializing using {browser_name} to intercept downloads...",
            0,
            total,
        )

        def create_driver(version=None):
            # Kill any lingering Chrome background tasks to avoid file locks
            try:
                import subprocess
        ...
        try:
            try:
                ...
                error_msg = str(e)
                # Auto-fix version only applies to Chrome/Brave
                if (
            ...
            # Block downloads behavior for all Chromium-based browsers (Chrome, Edge, Brave)
            ...
            for i, link in enumerate(links, 1):
                filename = link.split("#")[-1] if "#" in link else link.split("/")[-1]
                self.root.after(
                    0, self.update_ui, f"Processing [{i}/{total}]: {filename}"
                )
                ...
                try:
                    ...
                    # Clear old performance logs before clicking/waiting
                    # Clear old performance logs safely
                    if browser_name.lower() != "firefox":
                        try:
                            log_method = getattr(driver, "get_log", None)
                            if log_method:
                                log_method("performance")
                        except:
                            pass

                    # Give it time to bypass Cloudflare and look for the download trigger
                    for _ in range(30):
                        time.sleep(1)
                        if link_handled:
                            break

                        # 1. Check if the browser URL itself redirected to the final download link
                        current_url = driver.current_url
                        if "dl.fuckingfast.co" in current_url and current_url != link:
                            direct_url = current_url
                            break

                        # 2. Try to automatically click any download/continue button if it's visible
                        try:
                        ...
                        # 3. Check network performance logs for a redirect to the direct file server
                        if browser_name.lower() != "firefox":
                            try:
                                log_method = getattr(driver, "get_log", None)
                                if log_method:
                                    logs = log_method("performance")
                                    for log in logs:
                                        message = log.get("message")
                                        if message and "dl.fuckingfast.co" in message:
                                            match_net = re.search(
                                                r'(https://dl\.fuckingfast\.co/dl/[^\s"]+)',
                                                message,
                                            )
                                            if match_net:
                                                direct_url = match_net.group(1).split(
                                                    '"'
                                                )[0]
                                                link_handled = True
                                                break
                    ...
                    if direct_url:
                        ...
                        # --- CLEANUP BLOCK: Close extra tabs and cancel downloads ---
                        try:
                            # 1. Close any popup tabs, keep only the main window
                            ...
                            # 2. Cancel active downloads by navigating to Chrome's download manager and clearing them
                            driver.get("chrome://downloads/")
                            time.sleep(0.5)
                            # Execute JavaScript to remove all items from the download list and cancel ongoing ones
                            driver.execute_script(
                                """
                                const downloadsManager = document.querySelector('downloads-manager');
                                if (downloadsManager) {
                                    const shadowRoot = downloadsManager.shadowRoot;
                                    const items = shadowRoot.querySelectorAll('downloads-item');
                                    items.forEach(item => {
                                        const removeBtn = item.shadowRoot.querySelector('#remove');
                            ...
                            pass  # Ignore if download cleanup fails so it doesn't break the loop
                        # -------------------------------------------------------------
